...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        if (snap.exists())
          setVotes(snap.data().results || { person1: 0, person2: 0 });
      },
  ...
  const handleJoin = async (overrideCode, currentUser = user) => {
    ...
    const codeToUse =
      typeof overrideCode === 'string' ? overrideCode : inputCode;
  ...
  return (
    <div
      className="min-h-screen font-sans bg-[#f8f9fb] text-slate-800"
      dir="rtl"
    >
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-16">
        {/* 1. واجهة دخول الجمهور (مطابقة للصورة الأولى) */}
        {view === 'home' && (
          <div className="flex flex-col items-center animate-in fade-in zoom-in-95 duration-500 mt-10 md:mt-20">
            {/* عنوان التطبيق المخفي فيه الزر السري */}
            <h1
              onClick={handleSecretClick}
              className="text-3xl md:text-4xl font-bold text-[#1a56db] mb-10 tracking-wide cursor-default select-none"
            >
        ...
        {/* 2. واجهة التصويت (مطابقة للصورة الثانية تماماً) */}
        {view === 'vote' && pollData && (
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-500">
            ...
            <div className="grid grid-cols-2 gap-4 md:gap-8 mb-6">
              ...
              <div className="bg-white rounded-[24px] p-4 shadow-sm border border-slate-100 flex flex-col">
                <div className="w-full aspect-square bg-[#111] rounded-[16px] overflow-hidden relative mb-5">
                  <img
                    src={pollData.c1.image}
                    alt={pollData.c1.name}
            ...
            {/* تحليل النتائج (المطابق للتصميم السفلي) */}
        ...
        {/* 3. شاشة تسجيل دخول الإدارة (مخفية للعامة) */}
        {view === 'admin_login' && (
          <div className="w-full max-w-sm mx-auto animate-in zoom-in-95 duration-300 mt-20">
            <div className="bg-white p-8 rounded-[24px] shadow-sm border border-slate-100 text-center">
              ...
              <button
                ...
                className="text-slate-400 text-sm hover:text-slate-600 font-bold"
              >
                إلغاء
              </button>
            </div>
        ...
        {view === 'admin_dashboard' && (
          <div className="animate-in fade-in duration-500">
            <div className="flex justify-between items-center mb-8 bg-white p-4 rounded-2xl shadow-sm">
              ...
              <button
                onClick={() => setView('home')}
                className="bg-slate-100 px-4 py-2 rounded-lg text-slate-600 font-bold text-sm"
              >
                خروج
              </button>
            </div>
                ...
                <div className="w-full aspect-square bg-slate-50 rounded-2xl border-2 border-dashed border-slate-200 flex items-center justify-center relative overflow-hidden mb-4 cursor-pointer hover:bg-slate-100">
                  {c1.image ? (
                    ...
                    <div className="text-center">
                      <Icons.Upload />
                      <span className="text-slate-500 text-sm">اختر صورة</span>
                    </div>
                  )}
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => handleImg(e, setC1)}
            ...
            {adminError && (
              <p className="text-red-500 text-center mb-6 font-bold">
                {adminError}
              </p>
            )}
            ...
            {createdCode && (
              <div className="mt-8 bg-white border border-slate-100 shadow-sm p-8 rounded-[24px] text-center animate-in zoom-in-95">
                ...
                <div className="bg-slate-50 rounded-xl py-6 mb-6">
                  <h1 className="text-5xl font-black text-slate-800 tracking-[0.3em] font-mono">
                    {createdCode}
                  </h1>
                </div>
                ...
                <div className="max-w-md mx-auto">
                  ...
                  <div className="flex items-center gap-2 bg-slate-50 p-2 rounded-xl">
                    ...
                    <button
                      onClick={() =>
                        handleCopyLink(
                          `${window.location.origin}${window.location.pathname}?poll=${createdCode}`
                        )
                      }
